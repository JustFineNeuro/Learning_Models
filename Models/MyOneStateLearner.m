...
f_fname=@f_onestatemodel; %evoluation funciton
...
% options.inF = in;
options.inG.statemap = [1];
dim.n_theta = 2;%Number of evolution equation paramters.
dim.n_phi = 0;
dim.n = 1;%number of latentstates in evolution equation
dim.p = 1; disp('not sure what this is, but its for inversion?')
x0 = zeros(dim.n,1);

priors.muX0 = y(1);
priors.SigmaX0 = 1e-0*eye(1);
priors.muTheta = [0.8;0.95];
priors.SigmaTheta = 1e-1*eye(2);
priors.a_alpha = 1e0;
priors.b_alpha = 1e0;
priors.a_sigma = 1e0;
priors.b_sigma = 1e0;
...
% 
% alpha   = 3; %Precision on evolution parameters (alpha = hyperparameter)
% sigma   = 6; %Precision on observation parameters (alpha = hyperparameter)
% 
% [y,x,x0,eta,e] = simulateNLSS(nt,f_fname,g_fname,theta,[],ut',5,2,options,x0);
...
dim.n_t = nt; % trials (within-subject)
...
[y{i},x,x0,eta,e] = simulateNLSS(nt,f_fname,g_fname,theta,[],ut{i}',alpha,sigma,options,x0);
end
clear options
for i=1:ns
